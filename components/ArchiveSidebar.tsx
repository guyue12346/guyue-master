...
export const ArchiveSidebar: React.FC<ArchiveSidebarProps> = ({ 
  ...
  return (
    <div className="theme-sidebar-surface w-64 h-full flex-shrink-0 flex flex-col pt-6 pb-4 px-3 z-20">
        ...
        {Object.entries(groupedArchives).length === 0 ? (
          ...
          Object.entries(groupedArchives).map(([folderName, { files, category }]) => (
            <div key={folderName}>
              {/* Folder Item */}
              <div 
                className="theme-list-item flex items-center gap-2 px-2 py-1.5 cursor-pointer transition-colors group"
                onClick={() => toggleFolder(folderName)}
              >
                {expandedFolders.has(folderName) ? (
                  <ChevronDown className="w-3.5 h-3.5" style={{ color: 'var(--t-text-muted)' }} />
                ) : (
                    ...
                    >
                      ...
                      <div className={`flex gap-1 shrink-0 ${activeFileId === file.id ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                        <button 
                          ...
                          className="theme-icon-btn p-1"
                        >
                          <Edit2 className="w-3 h-3" />
                        </button>
                        <button 
                          onClick={(e) => { e.stopPropagation(); onDelete(file.id); }} 
                          className="theme-icon-btn theme-icon-btn-danger p-1"
                        >
