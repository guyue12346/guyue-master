...
const AGENT_MODULES: AgentModule[] = [
  ...
  { id: 'files',      name: '文件管理', icon: FolderOpen,  enabled: true,  description: '读取、创建文件（需授权文件夹）' },
...
10. **文件管理** — 列出目录、读取文件内容、创建新文件（需用户授权文件夹）
...
\`\`\`action
{
  "type": "read_file",
  "data": {
    "path": "/Users/xxx/Documents/notes.md"
  }
}
\`\`\`
...
\`\`\`action
{
  "type": "create_file",
  "data": {
    "path": "/Users/xxx/Documents/new-file.md",
    "content": "# 新文件\\n\\n内容..."
  }
}
\`\`\`
...
const SUPPORTED_ACTION_TYPES = new Set(['create_todo', 'create_note', 'create_prompt', 'create_markdown_note', 'create_oj_submission', 'create_resource', 'create_leetcode_list', 'create_learning_course', 'update_todo', 'delete_todo', 'update_note', 'delete_note', 'update_resource', 'delete_resource', 'send_email', 'list_files', 'read_file', 'create_file', 'query_images', 'upload_image', 'query_subtasks', 'create_subtask', 'update_subtask', 'delete_subtask']);
...
const TOOL_REGISTRY: ToolRegistration[] = [
  ...
  // ─── 文件管理工具（需文件夹授权）───
  {
    ...
    tool: {
      name: 'list_files',
      description: '列出指定目录下的文件和子目录。目录必须在用户已授权的文件夹范围内。',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    execute: async (args, ctx) => {
      ...
      const allowed = ctx.filePermissions.some(fp => {
        const nfp = fp.endsWith('/') ? fp : fp + '/';
        return normalized.startsWith(nfp) || nfp.startsWith(normalized);
      });
      if (!allowed) return { success: false, error: `目录「${dirPath}」未授权。请在右侧边栏「文件夹权限」中添加此目录或其父目录。当前已授权：${ctx.filePermissions.join('、') || '无'}` };
      const electronAPI = (window as any).electronAPI;
      if (!electronAPI?.listDir) return { success: false, error: '文件操作不可用（非桌面端）。' };
      const entries = await electronAPI.listDir(dirPath);
      if (!entries || entries.length === 0) return { success: true, path: dirPath, entries: [], message: '目录为空或不存在。' };
      return {
        success: true,
        path: dirPath,
        entries: entries.map((e: any) => ({ name: e.name, isDirectory: e.isDirectory, path: e.path })),
  ...
  },
  {
    ...
    tool: {
      name: 'read_file',
      description: '读取指定文件的文本内容。文件必须位于用户已授权的文件夹范围内。适合读取代码、文本、配置文件等。',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    execute: async (args, ctx) => {
      ...
      const allowed = ctx.filePermissions.some(fp => {
        const nfp = fp.endsWith('/') ? fp : fp + '/';
        return filePath.startsWith(nfp);
      });
      if (!allowed) return { success: false, error: `文件「${filePath}」不在已授权的文件夹范围内。当前已授权：${ctx.filePermissions.join('、') || '无'}` };
      const electronAPI = (window as any).electronAPI;
      if (!electronAPI?.readFile) return { success: false, error: '文件操作不可用（非桌面端）。' };
      const content = await electronAPI.readFile(filePath);
      if (content === null) return { success: false, error: `读取失败：文件不存在或无法读取「${filePath}」` };
      const MAX_LEN = 50000;
  ...
  },
  {
    ...
    tool: {
      name: 'create_file',
      description: '在指定路径创建新文件并写入内容。目标目录必须在用户已授权的文件夹范围内。如果文件已存在会被覆盖。',
      inputSchema: {
        type: 'object',
        properties: {
          path: { type: 'string', description: '文件的绝对路径，如 /Users/xxx/notes/memo.md' },
    ...
    execute: async (args, ctx) => {
      ...
      if (!allowed) return { success: false, error: `目标路径「${filePath}」不在已授权的文件夹范围内。当前已授权：${ctx.filePermissions.join('、') || '无'}` };
      const electronAPI = (window as any).electronAPI;
      if (!electronAPI?.writeFile || !electronAPI?.ensureDir) return { success: false, error: '文件操作不可用（非桌面端）。' };
      // 确保父目录存在
      const lastSlash = filePath.lastIndexOf('/');
      if (lastSlash > 0) {
        await electronAPI.ensureDir(filePath.substring(0, lastSlash));
      }
      const ok = await electronAPI.writeFile(filePath, content);
      if (!ok) return { success: false, error: `写入失败：「${filePath}」` };
      return { success: true, message: `文件已创建：${filePath}`, path: filePath, size: content.length };
    },
  },
  // ─── 图床工具 ───
  {
...
export const AgentPanel: React.FC<AgentPanelProps> = ({
        ...
        if (action?.status === 'pending') {
          try {
            ...
            if (action.type === 'create_todo') {
              ...
              const d = action.data || {};
              const dirPath = String(d.path || '').trim();
              if (!dirPath) throw new Error('路径不能为空。');
              const normalized = dirPath.endsWith('/') ? dirPath : dirPath + '/';
              const allowed = filePermissions.some(fp => {
                const nfp = fp.endsWith('/') ? fp : fp + '/';
                return normalized.startsWith(nfp) || nfp.startsWith(normalized);
              });
              if (!allowed) throw new Error(`目录「${dirPath}」未授权。请在右侧「文件夹权限」中添加。`);
              const eAPI = (window as any).electronAPI;
              if (!eAPI?.listDir) throw new Error('文件操作不可用（非桌面端）。');
              const entries = await eAPI.listDir(dirPath);
              resultData = { path: dirPath, entries: entries || [], count: (entries || []).length };
              summaryText = `目录列出成功：${(entries || []).length} 项`;
              ...
              if (!allowed) throw new Error(`文件「${filePath}」不在已授权的文件夹范围内。`);
              const eAPI = (window as any).electronAPI;
              if (!eAPI?.readFile) throw new Error('文件操作不可用（非桌面端）。');
              const content = await eAPI.readFile(filePath);
              if (content === null) throw new Error(`读取失败：文件不存在或无法读取。`);
              resultData = { path: filePath, content: content.length > 50000 ? content.slice(0, 50000) : content, length: content.length };
              summaryText = `文件读取成功：${filePath}`;
              ...
              if (!allowed) throw new Error(`目标路径「${filePath}」不在已授权的文件夹范围内。`);
              const eAPI = (window as any).electronAPI;
              if (!eAPI?.writeFile || !eAPI?.ensureDir) throw new Error('文件操作不可用（非桌面端）。');
              const lastSlash = filePath.lastIndexOf('/');
              if (lastSlash > 0) await eAPI.ensureDir(filePath.substring(0, lastSlash));
              const ok = await eAPI.writeFile(filePath, content);
              if (!ok) throw new Error(`写入失败：「${filePath}」`);
              resultData = { path: filePath, size: content.length };
              summaryText = `文件创建成功：${filePath}`;
  ...
  }, [config.model, config.provider, config.systemPrompt, messages, todos, dataPermissions, onCreateTodo, onCreateNote, onCreatePrompt, onCreateMarkdownNote, onCreateOJSubmission, ojHeatmapData, onCreateResource, resourceData, pushDebugItem, selectedModule]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const toolExecContext: ToolExecutionContext = { todos, notes, dataPermissions, filePermissions, lastUserAttachments: currentAttachments, onCreateTodo, onUpdateTodo, onDeleteTodo, onCreateNote, onUpdateNote, onDeleteNote, onCreatePrompt, onCreateMarkdownNote, onCreateOJSubmission, ojHeatmapData, onCreateResource, onUpdateResource, onDeleteResource, resourceData };
              ...
              <div className="relative">
                ...
                {showFilePermissions && (
                  <>
                    ...
                    <div className="absolute right-full top-0 mr-2 z-50 w-72 bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
                      <div className="px-4 pt-3.5 pb-2 border-b border-gray-100 flex items-center justify-between">
                        <p className="text-xs font-semibold text-gray-800">Agent 文件夹权限</p>
                        <button
                          onClick={async () => {
                            const electronAPI = (window as any).electronAPI;
                            if (!electronAPI?.selectDirectory) return;
                            const dir = await electronAPI.selectDirectory();
                            if (dir && !filePermissions.includes(dir)) {
                              setFilePermissions(prev => [...prev, dir]);
                            }
                          }}
                      ...
                      <div className="py-1.5 max-h-48 overflow-y-auto">
                        {filePermissions.length === 0 ? (
                          <p className="px-4 py-3 text-[11px] text-gray-400 text-center">未授权任何文件夹<br />Agent 无法访问本地文件</p>
                        ) : (
                          filePermissions.map((fp, idx) => (
                            <div key={idx} className="flex items-center gap-2 px-4 py-1.5 hover:bg-gray-50 group">
                              <FolderOpen className="w-3.5 h-3.5 text-green-500 shrink-0" />
                              <span className="flex-1 text-[11px] text-gray-700 truncate" title={fp}>{fp}</span>
                              <button
                                onClick={() => setFilePermissions(prev => prev.filter((_, i) => i !== idx))}
                                className="opacity-0 group-hover:opacity-100 w-5 h-5 flex items-center justify-center rounded text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all"
                                title="移除"
                              >
                                <X className="w-3 h-3" />
                              </button>
        ...
        <div className="flex items-center gap-2 flex-wrap mb-2">
          {message.action && (() => {
            const actionLabels: Record<string, { icon: typeof ListTodo; label: string }> = {
              ...
              list_files: { icon: FolderOpen, label: '目录列表' },
              read_file: { icon: FileText, label: '读取文件' },
              create_file: { icon: FilePlus2, label: '创建文件' },
