...
### 可用工具
- **create_todo** — 创建待办。必填 content（标题），可选 description、priority（high/medium/low）、category、dueDate（ISO 格式）。
- **query_todos** — 查询待办列表。可按 status（pending/completed/all）筛选，默认返回未完成项。
- **update_todo** — 修改待办。通过 id 定位，可改 content、priority、category、dueDate、isCompleted。
- **delete_todo** — 删除待办。通过 id 定位。
...
2. **设定截止日期**：用户说"明天下午三点"时，你必须自行计算并转为 ISO 格式（如 2026-03-16T15:00），不指定时间则默认 23:59。
...
- 设置 dueDate 时，你必须自行将用户描述的日期时间换算为 ISO 8601 格式（YYYY-MM-DDTHH:mm）。例如用户说"明天下午两点"，你应传入 "${new Date(Date.now() + 86400000).toISOString().split('T')[0]}T14:00"。如果用户没指定具体时间，默认使用 23:59，如 "${new Date(Date.now() + 86400000).toISOString().split('T')[0]}T23:59"。绝对不要传自然语言或时间戳。${modulePrompt}${customPromptSection}`;
  }
...
const TOOL_REGISTRY: ToolRegistration[] = [
  {
    ...
    tool: {
      name: 'create_todo',
      description: '创建一个新的待办事项。创建前必须先 query_todos 获取可用分类列表（availableCategories），category 必须是其中之一，不可自行编造。',
      inputSchema: {
        type: 'object',
        properties: {
...
const normalizeTodoPayload = (data: Record<string, any>): Partial<TodoItem> => {
  const dueDate = parseDueDateString(data.dueDate);
  // 如果 dueDate 字符串含有时刻部分（如 "2026-03-15T14:00"），说明是具体时间点，
  // 需要设置 timeType='point'，否则 ScheduleView 会将其识别为全天事件。
  const hasTime = typeof data.dueDate === 'string' && data.dueDate.includes('T');
  return {
    ...
    dueDate,
    ...(dueDate && hasTime ? { timeType: 'point' as const } : {}),
  };
};
